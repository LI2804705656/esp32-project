...
constexpr int LEFT_VIBRATION_PIN = 25;
constexpr int RIGHT_VIBRATION_PIN = 26;
...
void updateVibrationOutputs(bool alarmActive, const char *posture);
...
void updateVibrationOutputs(bool alarmActive, const char *posture)
{
  ...
  if (strcmp(posture, "左倾") == 0) {
    digitalWrite(LEFT_VIBRATION_PIN, HIGH);
    digitalWrite(RIGHT_VIBRATION_PIN, LOW);
  } else if (strcmp(posture, "右倾") == 0) {
    digitalWrite(LEFT_VIBRATION_PIN, LOW);
    digitalWrite(RIGHT_VIBRATION_PIN, HIGH);
  } else if (strcmp(posture, "前倾") == 0 || strcmp(posture, "后仰") == 0) {
    digitalWrite(LEFT_VIBRATION_PIN, HIGH);
    digitalWrite(RIGHT_VIBRATION_PIN, HIGH);
  } else {
    digitalWrite(LEFT_VIBRATION_PIN, LOW);
...
void loop()
{
  ...
  static const char *pendingPosture = "正常";
  ...
  const bool overThreshold = strcmp(posture, "正常") != 0;

  if (overThreshold) {
    if (tiltStartMs == 0 || strcmp(posture, pendingPosture) != 0) {
      tiltStartMs = millis();
  ...
  updateVibrationOutputs(alarmActive, posture);
  ...
  oled.drawUTF8(88, 31, alarmActive ? "震动" : "监测");
